...
def load_data():
    ...
    # ── fill all numeric cols before any calculations ──────────────────────
    for col in ["revenue", "cogs", "labour", "be_allocation",
                "ae_allocation", "rta_allocation", "gross_margin"]:
        df_gm[col] = df_gm[col].fillna(0)

    df_gm["contribution"] = df_gm["gross_margin"] - df_gm["labour"]
    df_gm["fixed_cost"] = (
        df_gm["be_allocation"] + df_gm["ae_allocation"] + df_gm["rta_allocation"]
    )
    # rest of load_data unchanged...
...
with st.sidebar:
    ...
    # 2. Sub Service Line — narrowed by SL
    ...
    # 4. Client — narrowed by SL + SSL + Vertical
    client_pool = vert_pool if selected_vertical == "All" else vert_pool[
    vert_pool["vertical_name"] == selected_vertical]
...
with tab1:
    ...
    product_summary = product_summary.sort_values(
        ["Service Line Name", "Sub Service Line Name"], ascending=[True, True]
    )
