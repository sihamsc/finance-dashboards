WITH fixed_costs AS (
    SELECT 2022 AS yr, 1000 AS fixed_cost_ae
...
ae_revenue_by_customer AS (
    -- Annual Ad Effect revenue per top level customer + vertical + year
...
tot_ae_revenue AS (
    -- Total Ad Effect revenue across ALL customers per year
    SELECT yr, SUM(annual_ae_rev) AS tot_ae_rev
    FROM ae_revenue_by_customer
    GROUP BY 1
),

allocation AS (
    -- Monthly allocation = (customer AE rev / total AE rev) * fixed cost / 12
    SELECT
        b.yr,
        ...
        f.fixed_cost_ae,
        ROUND(
            ((b.annual_ae_rev / NULLIF(t.tot_ae_rev, 0)) * f.fixed_cost_ae / 12)::numeric,
            2
        ) AS ae_allocation_per_month
    FROM ae_revenue_by_customer b
    JOIN tot_ae_revenue t ON b.yr = t.yr
...
SELECT
    ...
    a.annual_ae_rev,
    a.tot_ae_rev          AS annual_tot_ae_rev,
    a.fixed_cost_ae,
    a.ae_allocation_per_month
FROM allocation a
JOIN periods p ON a.yr = p.yr
ORDER BY p.accounting_period_start_date, a.ae_allocation_per_month DESC