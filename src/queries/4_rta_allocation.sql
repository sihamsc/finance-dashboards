WITH fixed_costs AS (
    SELECT 2022 AS yr, 1000 AS fixed_cost_rta
...
rta_revenue_by_customer AS (
    -- Annual Ad Solutions RTA revenue per top level customer + vertical + year
    -- Grouped at top_level_parent_customer_name to avoid subsidiary duplication
    SELECT
...
tot_rta_revenue AS (
    -- Total RTA revenue across ALL customers per year
    SELECT yr, SUM(annual_rta_rev) AS tot_rta_rev
    FROM rta_revenue_by_customer
    GROUP BY 1
),

allocation AS (
    -- Monthly allocation = (customer RTA rev / total RTA rev) * fixed cost / 12
    SELECT
        b.yr,
        ...
        f.fixed_cost_rta,
        ROUND(
            ((b.annual_rta_rev / NULLIF(t.tot_rta_rev, 0)) * f.fixed_cost_rta / 12)::numeric,
            2
        ) AS rta_allocation_per_month
    FROM rta_revenue_by_customer b
    JOIN tot_rta_revenue t ON b.yr = t.yr
    JOIN fixed_costs f      ON b.yr = f.yr
),

-- Fan out to all 12 months per year regardless of monthly activity
periods AS (
    SELECT DISTINCT
...
SELECT
    ...
    a.annual_rta_rev,
    a.tot_rta_rev          AS annual_tot_rta_rev,
    a.fixed_cost_rta,
    a.rta_allocation_per_month
FROM allocation a
JOIN periods p ON a.yr = p.yr
ORDER BY p.accounting_period_start_date, a.rta_allocation_per_month DESC