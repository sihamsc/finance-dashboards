...
with st.sidebar:
    st.markdown(
        ...
        f'<div><div class="sb-brand-name">MarketCast</div><div class="sb-brand-sub">Finance Dashboard</div></div></div>',
        unsafe_allow_html=True
    )
...
with tab1:
    ...
    st.markdown('<div class="section-header">Margin Detail — with Attributed Labor</div>', unsafe_allow_html=True)

    lab_sl = (
        df_lab_curr.groupby(["service_line_name","sub_service_line_name"])["labour_cost"]
        .sum()
        .reset_index()
        .rename(columns={"labour_cost":"labor_attributed"})
    )
    ps = (
    ...
    ps["labor_attributed"] = ps["labor_attributed"].fillna(0)
    ps["contribution"] = ps["gross_margin"] - ps["labor_attributed"]
    ...
    for col in ["revenue","cogs","fixed_cost","gross_margin","labor_attributed","contribution"]:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Labor Attributed":st.column_config.NumberColumn("Labor ($k)", format="$%.0f"),
...
with tab2:
    ...
    st.markdown(
        "<div style='font-family:DM Mono,monospace;font-size:9px;letter-spacing:0.12em;text-transform:uppercase;color:#6b7280;margin-bottom:0.35rem;'>Client Value Metric</div>",
        unsafe_allow_html=True
    )
    ...
    ca2["labor_attributed"] = ca2["labor_attributed"].fillna(0)
    ca2["contribution"] = ca2["gross_margin"] - ca2["labor_attributed"]
    ...
    with ch:
        ...
        f5 = px.scatter(
            ...
            title=f"Client Portfolio — Revenue vs {pt2}",
            size_max=36,
            labels={"revenue":"Revenue", pc:pt2, mc:mt, "top_level_parent_customer_name":""}
        )
        f5.update_traces(
            marker=dict(color=LINE_CURRENT, sizemode="area", opacity=0.9),
    ...
    slsum["labor_attributed"] = slsum["labor_attributed"].fillna(0)
    slsum["contribution"] = slsum["gross_margin"] - slsum["labor_attributed"]
    ...
    for _, row in slsum.iterrows():
        ...
        with st.expander(f"{sl_name}  |  Revenue: {fmt_m(row['revenue'])}  |  GM%: {row['gm_pct']:.1f}%  |  CM%: {row['cm_pct']:.1f}%"):
            ...
            sub_lab = (
                ...
                .groupby("sub_service_line_name")["labour_cost"]
                .sum()
                .reset_index()
                .rename(columns={"labour_cost":"labor_attributed"})
            )
            sub_gm = sub_gm.merge(sub_lab, on="sub_service_line_name", how="left")
            sub_gm["labor_attributed"] = sub_gm["labor_attributed"].fillna(0)
            sub_gm["contribution"] = sub_gm["gross_margin"] - sub_gm["labor_attributed"]
...
with tab3:
    ...
    lc1.markdown(kpi("Total Labor", tlab, tlab - tlab_py, "vs PY"), unsafe_allow_html=True)
    lc2.markdown(kpi("Labor % Revenue", safe_pct(tlab, rev), kind="pct"), unsafe_allow_html=True)
    lc3.markdown(kpi("Total Hours", thrs, kind="count"), unsafe_allow_html=True)
    lc4.markdown(kpi("Avg Cost/hr", tlab / thrs if thrs > 0 else 0, kind="dollar"), unsafe_allow_html=True)
    ...
    with cla:
        ...
        flm = px.bar(
            ...
            color="service_line_name",
            color_discrete_sequence=SERIES_COLORS,
            title="Labor by Service Line — Monthly",
            labels={"labour_cost":"Labor Cost","accounting_period_start_date":"Month","service_line_name":""}
        )
    ...
    with clb:
        lbc = (
            ...
            .groupby("top_level_parent_customer_name")["labour_cost"]
            .sum()
            .reset_index()
            .sort_values("labour_cost", ascending=True)
            .tail(15)
        )
        flc = px.bar(
            lbc,
            ...
            color="labour_cost",
            color_continuous_scale=BLUE_SCALE,
            title="Top 15 Clients by Labor Cost",
            labels={"labour_cost":"Labor Cost","top_level_parent_customer_name":""}
        )
    ...
    with clc:
        ...
        lrv["labor_pct"] = (lrv["labour_cost"] / lrv["revenue"].replace(0, float("nan")) * 100).round(1)
        lrv = lrv[lrv["revenue"] > 0].sort_values("labor_pct", ascending=True)
        flr = px.bar(
            lrv,
            x="labor_pct",
            y="service_line_name",
            ...
            color="labor_pct",
            color_continuous_scale=BLUE_SCALE,
            title="Labor % of Revenue by Service Line",
            labels={"labor_pct":"Labor %","service_line_name":""}
        )
    ...
    with cld:
        ...
        fly = px.bar(
            ...
            color="period",
            barmode="group",
            color_discrete_sequence=[LINE_CURRENT, LINE_PRIOR],
            title="Labor YoY by Service Line",
            labels={"labour_cost":"Labor Cost","service_line_name":"","period":"Year"}
        )
    ...
    st.markdown('<div class="section-header">Labor Detail — Client × Service Line</div>', unsafe_allow_html=True)
    ld = (
        df_lab_curr[~df_lab_curr["top_level_parent_customer_name"].isin(EXCL)]
    ...
    ld["labour_cost"] = (ld["labour_cost"] / 1_000_000).round(2)
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Labour Cost":st.column_config.NumberColumn("Labor ($M)", format="$%.2f"),
    ...
    with ck:
        if not dty.empty:
            ...
            f10 = px.bar(
                ...
                labels={"target_usd":"Target","ql":"Quarter"},
