...
with st.sidebar:
    st.markdown(f"""
    <div class="sb-brand">
        <img src="data:image/png;base64,{logo_base64}" class="sb-logo" />
        <div>
    ...
    default_index = years.index(2025) if 2025 in years else 0
    selected_year = st.selectbox("Year", years, index=default_index)
    ...
    service_lines = ["All"] + sorted([x for x in df_gm["service_line_name"].unique() if x != "(blank)"])
    ...
    sub_service_lines = ["All"] + sorted([x for x in ssl_pool["sub_service_line_name"].unique() if x != "(blank)"])
    selected_ssl = st.selectbox("Sub Service Line", sub_service_lines)

    vert_pool = ssl_pool if selected_ssl == "All" else ssl_pool[ssl_pool["sub_service_line_name"] == selected_ssl]
    verticals = ["All"] + sorted([x for x in vert_pool["vertical_name"].unique() if x != "(blank)"])
    selected_vertical = st.selectbox("Vertical", verticals)

    client_pool = vert_pool if selected_vertical == "All" else vert_pool[vert_pool["vertical_name"] == selected_vertical]
    customers = ["All"] + sorted([x for x in client_pool["top_level_parent_customer_name"].unique() if x not in EXCL])
...
r1[4].markdown(kpi("Labor", labor), unsafe_allow_html=True)
...
r2[2].markdown(kpi("Labor % of Revenue", labor_pct, labor_pct - labor_pct_py, "vs PY", kind="pct"), unsafe_allow_html=True)
...
with tab1:
    ...
    product_summary = product_summary.sort_values(["Service Line Name", "Sub Service Line Name"], ascending=[True, True])
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Labour": st.column_config.NumberColumn("Labor ($k)", format="$%.0f"),
...
with tab2:
    ...
    use_contribution = st.toggle(
        "Use Contribution",
        value=False,
        key="client_metric_toggle"
    )

    metric_col = "contribution" if use_contribution else "gross_margin"
    pct_col = "cm_pct" if use_contribution else "gm_pct"
    metric_title = "Contribution" if use_contribution else "Gross Margin"
    pct_title = "Contribution %" if use_contribution else "Gross Margin %"
    ...
    with col_g:
        ...
        fig4 = px.bar(
            ...
            title=f"Top 15 Clients by {metric_title}",
            labels={
    ...
    with col_h:
        ...
        client_perf["gm_pct"] = (
            client_perf["gross_margin"] / client_perf["revenue"].replace(0, float("nan")) * 100
        ).round(1)
        client_perf["cm_pct"] = (
            client_perf["contribution"] / client_perf["revenue"].replace(0, float("nan")) * 100
        ).round(1)
        ...
        fig5 = px.scatter(
            ...
            title=f"Client Portfolio — Revenue vs {pct_title}",
            labels={
                "revenue": "Revenue",
                pct_col: pct_title,
                metric_col: metric_title,
        ...
        fig5.update_layout(**PT, title_font_color="#cbd5e1", showlegend=False)
        ...
        with st.expander(
            ...
            service_metrics = pd.DataFrame({
                "Metric": [
                    ...
                    "Labor", "Contribution", "Gross Margin %", "Contribution %", "Clients"
                ],
                "Value": [
                    fmt_m(row["revenue"]),
                    fmt_m(row["cogs"]),
...
with tab4:
    ...
    with col_k:
        if not df_tgt_year.empty:
            q_targets = (
                df_tgt_year.assign(
                    quarter_label="Q" + df_tgt_year["quarter_start_date"].dt.quarter.astype(str)
                )
                .groupby(["quarter_label", "quarter_start_date"], as_index=False)["target_usd"]
                .sum()
                .sort_values("quarter_start_date")
            )
            quarter_order = q_targets["quarter_label"].tolist()
            ...
            fig10 = px.bar(
                ...
                labels={"target_usd": "Target", "quarter_label": "Quarter"},
